%------------------------------------------------------------------------%
% Billy Orion Mazotti and Kaitlyn Fa
...
% if city == "San Francisco"
    ...
    case "New York City"
        ...
        % customer/stop locations
        customer_file = "FormattedDatasets/NYC_formatted/Y2023_MJul_D24_h17_m56_s04_NYC_customers_N1082.mat";
        % customer_file = "FormattedDatasets/NYC_formatted/Y2023_MJul_D23_h22_m21_s24_NYC_customers_N1082.mat";
...
customer_plot = scatter(xyCustomers_local(:,1),xyCustomers_local(:,2),5,'filled', ...
    'DisplayName','customers','MarkerFaceColor',[0 1 1],'MarkerEdgeColor',[0 1 1]);
vendor_plot = scatter(xyVendors_local(:,1),xyVendors_local(:,2),50,'^', ...
    'DisplayName','vendors','MarkerFaceColor',[1 0 0],'MarkerEdgeColor',[0 0 0]);

legend([customer_plot vendor_plot]);

hold off;

xlabe('X [m]','Interpreter','Latex','FontSize',15)
ylabel('Y [m]','Interpreter','Latex','FontSize',15)
ax = ancestor(h, 'axes');
ax.XAxis.Exponent = 4;
ax.YAxis.Exponent = 4;
...
% Reachable Set Extraction Results % K.
figure();
...
% making a potential convexhull (reachable set)
reachableSetMatrix = [20000 20000; 30000 31000; 40000 20000; 30000 10000; 21000 13000];
k = convhull(reachableSetMatrix);
% [POTENTIAL OPTIMIZATION] finding the max and min lat/long of the reachable set

% extracting customers and vendors within reachable set
vendorsInReachableSet = extractBuildings(reachableSetMatrix, xyVendors_local);
customersInReachableSet = extractBuildings(reachableSetMatrix, xyCustomers_local);

% plot reachable set (convexhull)
reachableSet = plot(reachableSetMatrix(k, 1), reachableSetMatrix(k, 2), 'LineWidth', 1, 'DisplayName', 'reachable set');

% preparing data for plotting
xCustomers_local = xyCustomers_local(:, 1);
yCustomers_local = xyCustomers_local(:, 2);
xVendors_local = xyVendors_local(:, 1);
yVendors_local = xyVendors_local(:, 2);

% plot extracted data
customer_in_plot = scatter(xCustomers_local(customersInReachableSet),yCustomers_local(customersInReachableSet),5,'filled', ...
    'DisplayName','customers inside set','MarkerFaceColor',[0 1 1],'MarkerEdgeColor',[0 1 1])

vendor_in_plot = scatter(xVendors_local(vendorsInReachableSet),yVendors_local(vendorsInReachableSet),50,'^', ...
    'DisplayName','vendors inside set','MarkerFaceColor',[1 0 0],'MarkerEdgeColor',[0 0 0])

% plot data outside of the reachable set
customer_out_plot = scatter(xCustomers_local(~customersInReachableSet),yCustomers_local(~customersInReachableSet),5,'filled', ...
    'DisplayName','customers outside set','MarkerFaceColor',[0 1 0],'MarkerEdgeColor',[0 1 0])

vendor_out_plot = scatter(xVendors_local(~vendorsInReachableSet),yVendors_local(~vendorsInReachableSet),50,'^', ...
    'DisplayName','vendors outside set','MarkerFaceColor',[1 0 1],'MarkerEdgeColor',[0 0 0])

legend([reachableSet customer_in_plot vendor_in_plot customer_out_plot vendor_out_plot]);
hold off
...
ax.XAxis.Exponent = 4;
ax.YAxis.Exponent = 4;
sgtitle(city + ": Customers and Vendors Within the Reachable Set",'interpreter','latex','FontSize',12)
axis equal


% Finding Possible Paths Within the Reachable Set % K.
figure();

% % potential reachable set
% smallReachableSetMatrix = [11000 9000; 11000 15000; 19000 15000; 19000 9000];
% m = convhull(smallReachableSetMatrix);
...
% potential reachable set
smallReachableSetMatrix = [11000 9000; 11000 15000; 12500 17000; 19000 15000; 19000 9000; 14000 7000];
m = convhull(smallReachableSetMatrix);

% plot reachable set
smallReachableSet = plot(smallReachableSetMatrix(m, 1), smallReachableSetMatrix(m, 2), 'LineWidth', 1);

% extract data
TSP_vendorsInReachableSet = extractBuildings(smallReachableSetMatrix, xyVendors_local);
TSP_customersInReachableSet = extractBuildings(smallReachableSetMatrix, xyCustomers_local);

% prepare data for plotting
TSP_xCustomers_local = xyCustomers_local(:, 1);
TSP_yCustomers_local = xyCustomers_local(:, 2);
TSP_xVendors_local = xyVendors_local(:, 1); 
TSP_yVendors_local = xyVendors_local(:, 2);

% plot extracted data
TSP_customers_in_plot = scatter(TSP_xCustomers_local(TSP_customersInReachableSet),TSP_yCustomers_local(TSP_customersInReachableSet),5,'filled', ...
    'DisplayName','customers','MarkerFaceColor',[0 1 1],'MarkerEdgeColor',[0 1 1])

TSP_vendors_in_plot = scatter(TSP_xVendors_local(TSP_vendorsInReachableSet),TSP_yVendors_local(TSP_vendorsInReachableSet),50,'^', ...
    'DisplayName','vendors','MarkerFaceColor',[1 0 0],'MarkerEdgeColor',[0 0 0])

legend([TSP_customers_in_plot TSP_vendors_in_plot]);
% run TSP
...
ax.XAxis.Exponent = 4;
ax.YAxis.Exponent = 4;
sgtitle(city + ": Possible Paths Within the Reachable Set",'interpreter','latex','FontSize',12)
axis equal
...
% %% Experiment
% K. Commented because of no need
% 
...
% 1. seperate all functions in ./utl