...
function [S_out,num_customers] = CustomerStopLocations(shapeFilePath,numTableRows_raw, ...
    ...
    %{
        ...
        Returns:
            S_out:  struct with fields LocationType (= arg LocationType), 
                    LocationX (values in meters), LocationY (values in 
    ...
    num_customers = size(S_raw, 1);
    % xLocations(num_customers, 1);
    % yLocations(num_customers, 1);

    for i = 1:1:num_customers
        polyin = polyshape(S_raw(i).X, S_raw(i).Y);
        [x_in, y_in] = centroid(polyin);
        xLocations(i, 1) = x_in;
        yLocations(i, 1) = y_in;
    end

    % S_out.num_customers = num_customers;
